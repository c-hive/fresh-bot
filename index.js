...
module.exports = (app) => {
  ...
  const message =
    "Don't close this issue please. This is automatic message by [Yes, my issue is important](https://github.com/thisismydesign/yes-my-issue-is-important) - a bot against stable bots.";
  ...
  server.get("/login/cb", async (req, res) => {
    ...
    notifications.forEach(async (notification) => {
      const { latest_comment_url } = notification.subject;
      const subject_url = notification.subject.url;
      if (!latest_comment_url || !subject_url) {
        return;
      }

      const comment_response = await octokit
        .request(latest_comment_url)
        .catch(() => {});
      if (!comment_response) {
        return;
      }
      ...
      const { body } = comment_response.data;

      if (
        user.match(/\[bot\]$/) &&
        body.match(/^This issue has been automatically marked as stale/)
      ) {
